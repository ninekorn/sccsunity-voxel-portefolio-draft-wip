...
myself with a lot of trial and error and headaches. rev4 released and i developed a per frame voxel mesh 3d flat loop (xyz) that dictate how fast the per frame 3d flat loop has to loop to unqueue the Queues, and it's much faster than my previous revisions and it's smoother on the cpu to generate a voxel mesh although the neighbooring chunk check isn't working anymore:
