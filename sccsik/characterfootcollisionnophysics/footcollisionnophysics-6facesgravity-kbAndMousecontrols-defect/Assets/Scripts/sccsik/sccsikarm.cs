...
//[ExecuteInEditMode]
public class sccsikarm : MonoBehaviour
{
    public int swtcForMovingTarget = 1;
    //0 == move target
    //1 == don't move target
    ...
    public Transform handrenderer;
    public Transform renderupperarm;
    public Transform foreArmrenderer;
    public Transform shoulderrenderer;


    public GameObject prefabgameobjectempty;
    GameObject shoulderemptygameobject;
    GameObject upperarmemptygameobject;
    GameObject lowerarmemptygameobject;
    GameObject handemptygameobject;

    public int typeofArm = 0; //0 ==left arm , 1==rightarm

    public Vector3 shoulderoriginpositionoffset;

    Vector3 originshoulderpos;


    Quaternion originrotation;
    public Vector3 originroteulerangles = new Vector3(0, 0, 0);


    public void StartScript(Transform parent, GameObject ikarm)
    {

        originrotation = this.transform.rotation;

        this.transform.position = shoulderoriginpositionoffset;


        originshoulderpos = this.transform.position;
        ...
        /*
        pointer0 = GameObject.CreatePrimitive(PrimitiveType.Sphere).transform;// Instantiate(GameObject.CreatePrimitive(PrimitiveType.Cube), transform.position, Quaternion.identity);
        pointer0.localScale = new Vector3(0.1f, 0.1f, 0.1f);

        pointer1 = GameObject.CreatePrimitive(PrimitiveType.Sphere).transform;// Instantiate(GameObject.CreatePrimitive(PrimitiveType.Cube), transform.position, Quaternion.identity);
        pointer1.localScale = new Vector3(0.1f, 0.1f, 0.1f);

        pointer2 = GameObject.CreatePrimitive(PrimitiveType.Sphere).transform;// Instantiate(GameObject.CreatePrimitive(PrimitiveType.Cube), transform.position, Quaternion.identity);
        pointer2.localScale = new Vector3(0.1f, 0.1f, 0.1f);

        pointer3 = GameObject.CreatePrimitive(PrimitiveType.Sphere).transform;// Instantiate(GameObject.CreatePrimitive(PrimitiveType.Cube), transform.position, Quaternion.identity);
        pointer3.localScale = new Vector3(0.1f, 0.1f, 0.1f);*/


        prefabgameobjectempty = new GameObject();

        prefabgameobjectempty.transform.name = "sccsikarm:" + typeofArm;
        prefabgameobjectempty.transform.position = this.transform.position;// shoulderoriginpositionoffset;
        prefabgameobjectempty.transform.parent = parent;
        //prefabgameobjectempty.transform.position = shoulderoriginpositionoffset;

        //this.transform.position = shoulderoriginpositionoffset;


        shoulder = new GameObject().transform;
        shoulder.transform.position = prefabgameobjectempty.transform.position;
        //shoulder.transform.position = shoulderoriginpositionoffset + originshoulderpos;
        shoulder.transform.parent = prefabgameobjectempty.transform;
        shoulder.transform.name = "shoulder";



        shoulderrenderer = GameObject.CreatePrimitive(PrimitiveType.Cube).transform;
        shoulderrenderer.localScale = new Vector3(0.1f, 0.1f, 0.1f);
        ...
        //upperarmemptygameobject = new GameObject();
        this.transform.parent = shoulder.transform;
        this.transform.position = shoulderrenderer.transform.position + new Vector3(0, 0, 0);// ;// new Vector3(-0.1f, -0.1f, 0.35f / 2);// new Vector3(-0.1f, 0, 0.0f);
        this.transform.name = "upperArm";
        upperArm = this.transform;

        renderupperarm = GameObject.CreatePrimitive(PrimitiveType.Cube).transform;
        renderupperarm.localScale = new Vector3(0.1f, 0.1f, 0.35f);


        if (typeofArm == 0)
        {
            renderupperarm.transform.position = this.transform.position + new Vector3(0, 0, 0.35f / 2) + new Vector3(0, 0, shoulderrenderer.localScale.y * 0.5f);
        }
        else if (typeofArm == 1)
        {
            renderupperarm.transform.position = this.transform.position + new Vector3(0, 0, 0.35f / 2) + new Vector3(0, 0, shoulderrenderer.localScale.y * 0.5f);
        ...
        renderupperarm.transform.parent = this.transform;
        renderupperarm.transform.name = "upperArmrenderer";
        ...
        foreArm = new GameObject().transform;
        foreArm.transform.parent = upperArm.transform;
        foreArm.transform.position = renderupperarm.transform.position + new Vector3(0, 0, (renderupperarm.transform.localScale.z) / 2);//new Vector3(-0.1f, -0.1f, 0.35f + (0.25f / 2) + 0.015f);// new Vector3(-0.1f, 0, 0.35f);
        foreArm.transform.name = "foreArm";


        foreArmrenderer = GameObject.CreatePrimitive(PrimitiveType.Cube).transform;
        foreArmrenderer.localScale = new Vector3(0.1f, 0.1f, 0.25f);
        foreArmrenderer.transform.position = foreArm.transform.position + new Vector3(0, 0, foreArmrenderer.localScale.z * 0.5f);// new Vector3(-0.1f, -0.1f, 0.35f + (0.25f / 2) + 0.015f);
        foreArmrenderer.transform.parent = foreArm.transform;
        foreArmrenderer.transform.name = "foreArmrenderer";
        ...
        hand = new GameObject().transform;
        hand.transform.parent = foreArm.transform;
        hand.transform.position = foreArmrenderer.transform.position + new Vector3(0, 0, (foreArmrenderer.transform.localScale.z / 2) + 0.05f);// new Vector3(-0.1f, -0.1f, 0.35f + (0.25f) + 0.075f);// hand.transform.position + new Vector3(0, 0, 0.1f);// new Vector3(-0.1f, 0, 0.35f + 0.25f);
        hand.transform.name = "hand";

        handrenderer = GameObject.CreatePrimitive(PrimitiveType.Cube).transform;
        handrenderer.localScale = new Vector3(0.1f, 0.1f, 0.1f);
        ...
        endForeArm.transform.parent = foreArmrenderer.transform;
        endForeArm.transform.position = hand.transform.position - new Vector3(0, 0, 0.05f);// + new Vector3(0, 0, 0.1f);
        endForeArm.transform.name = "endForeArm";

        endHand = new GameObject();
        endHand.transform.parent = handrenderer.transform;
        endHand.transform.position = handrenderer.transform.position + new Vector3(0, 0, 0.05f);
        endHand.transform.name = "endHand";



        if (typeofArm == 1)
        {
            elbowTarget = new GameObject();
            //elbowTarget.transform.parent = hand.transform;
            elbowTarget.transform.position = endForeArm.transform.position + new Vector3(-0.1f, -0.1f, 0) + new Vector3(1, -1, 0);// + new Vector3(0, 0, 0.1f);
            elbowTarget.transform.name = "elbowTarget";

            handTarget = new GameObject();
            //handTarget.transform.parent = hand.transform;
            handTarget.transform.position = handrenderer.transform.position + (-Vector3.forward * 0.35f) + (-Vector3.up * 0.45f) + (Vector3.right * 0.15f);// + new Vector3(0, 0, 0.1f);
            handTarget.transform.name = "HandTarget";
        }
        else if (typeofArm == 0)
        {
            elbowTarget = new GameObject();
        ...
        else if (typeofArm == 3)
        {
            ...
            handTarget = new GameObject();
            //handTarget.transform.parent = hand.transform;
            handTarget.transform.position = upperArm.transform.position + (-Vector3.up * 0.5f);// + new Vector3(0, 0, 0.1f);
            handTarget.transform.name = "HandTarget";
        }
        else if (typeofArm == 2)
        {
        ...
        else if (typeofArm == 2)
        {
            ...
            handTarget = new GameObject();
            //handTarget.transform.parent = hand.transform;
            handTarget.transform.position = handrenderer.transform.position + new Vector3(0, 0, -0.25f);// + new Vector3(0, 0, 0.1f);
            handTarget.transform.name = "HandTarget";
        }
        else if (typeofArm == 3)
        {
        ...
        //handTarget.transform.position -= prefabgameobjectempty.transform.position;

        Vector3 dirdiff = handTarget.transform.position - prefabgameobjectempty.transform.position;

        //handTarget.transform.position += dirdiff;



        elbowTarget.transform.parent = prefabgameobjectempty.transform;
        handTarget.transform.parent = prefabgameobjectempty.transform;
        ...
        //prefabgameobjectempty.transform.rotation = Quaternion.EulerRotation(originroteulerangles);// originrotation;

        //prefabgameobjectempty.transform.rotation *= Quaternion.EulerRotation(originroteulerangles);

        //prefabgameobjectempty.transform.rotation = Quaternion.EulerRotation(originroteulerangles).normalized;
        ...
        prefabgameobjectempty.transform.position = rot * (prefabgameobjectempty.transform.position - shoulder.transform.position) + shoulder.transform.position;
        prefabgameobjectempty.transform.rotation = rot * prefabgameobjectempty.transform.rotation;
        */
        ...
        if (swtcforcontinue == 1)
        {
            ...
            //lengthHand = Vector3.Distance(hand.position + new Vector3(0, 0, handrenderer.transform.localScale.z * 0.5f), foreArm.transform.position + new Vector3(0, 0, foreArmrenderer.transform.localScale.z * 0.5f));// Vector3.Distance(hand.position, endHand.transform.position);

            //lengthHand = 0.1f;// Vector3.Distance(hand.position + new Vector3(0, 0, handrenderer.transform.localScale.z * 0.5f), foreArm.transform.position);//
    ...
    void LateUpdate()
    {
        ...
        if (swtcforcontinue == 1)
        {
            ...
            dirElbowTargetToHandTarget = handTarget.transform.position - elbowTarget.transform.position;
            
            var dirLowerArmToHandTarget = handTarget.transform.position - foreArm.transform.position;

            float distshoulderToHandtarget = Vector3.Distance(shoulder.position, handTarget.transform.position);
            ...
            */
            ...
            if (swtcForMovingTarget == 0)
            {
                ...
                elbowTarget.transform.position = locOfPointadjacentX + (crosser.normalized);
                //to readd if it doesn't work
            }
