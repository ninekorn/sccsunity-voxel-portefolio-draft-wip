        ...
        for (int x = -chunkwleft; x <= chunkwright; x++)
        {
            for (int y = -chunkhleft; y <= chunkhright; y++)
            {
                for (int z = -chunkdleft; z <= chunkdright; z++)
                {
                    ...
                    arrayofplanetdiv[theindex].transform.position = new Vector3((x) * (20 *( chunkwleft+chunkwright+1)) * planesize, y * (20 * (chunkhleft + chunkhright + 1)) * planesize, z * (20 * (chunkdleft + chunkdright + 1)) * planesize);
                    ...
                    arrayofplanetdiv[theindex].width = 20;
                    arrayofplanetdiv[theindex].height = 20;
                    arrayofplanetdiv[theindex].depth = 20;

                    arrayofplanetdiv[theindex].iterateloopmap = 20;
                    arrayofplanetdiv[theindex].iterateloopmesh = 1;
                    arrayofplanetdiv[theindex].iterateonthreadloop = 20;
