...
public class sccsproceduralplanetbuilderrev2 : MonoBehaviour
{
        ...
        for (int x = -ChunkWidth_L; x <= ChunkWidth_R; x += 4)
        {
            for (int y = -ChunkHeight_L; y <= ChunkHeight_R; y += 4)
            {
                for (int z = -ChunkDepth_L; z <= ChunkDepth_R; z += 4)
                {
                    ...
                    blockers[_index].planetchunk.GetComponent<sccsplanetchunkrev2>().buildchunkmap();
                }
            }
        }
        ...
        for (int x = -ChunkWidth_L; x <= ChunkWidth_R; x += 4)
        {
            for (int y = -ChunkHeight_L; y <= ChunkHeight_R; y += 4)
            {
                for (int z = -ChunkDepth_L; z <= ChunkDepth_R; z += 4)
                {
                    ...
                    blockers[_index].planetchunk.GetComponent<sccsplanetchunkrev2>().Regenerate();
                    blockers[_index].planetchunk.GetComponent<sccsplanetchunkrev2>().buildMesh();
                }
            }
        }
