        ...
        for (int mx = 0; mx < levelsizex; mx++)
        {
            for (int my = 0; my < levelsizey; my++)
            {
                for (int mz = 0; mz < levelsizez; mz++)
                {
                    ...
                    computeVertexes.SetBuffer(0, "mapfirstvertxtop", mapvertlocbufferx);
                    //computeVertexes.SetBuffer(0, "mapfirstvertytop", mapvertlocbuffery);
                    computeVertexes.SetBuffer(0, "mapfirstvertztop", mapvertlocbufferz);

                    computeVertexes.SetBuffer(0, "widthdimtop", mapwidthdimtop);
                    computeVertexes.SetBuffer(0, "heightdimtop", mapheightdimtop);
                    computeVertexes.SetBuffer(0, "depthdimtop", mapdepthdimtop);
