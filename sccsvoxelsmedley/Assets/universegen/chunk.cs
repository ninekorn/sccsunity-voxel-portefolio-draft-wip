...
public class chunk: MonoBehaviour
{
    ...
    public universemaponly.meshData startBuildingArray(Vector3 currentPosition,int smallX, int smallY, int smallZ,int[] mapper)
    {
        ...
        if (positions.Length > 0)
        {
            ...
            return new universemaponly.meshData(currentPosition, positions.Length, positions, triangleIndices,smallX,smallY,smallZ);
        }
        else
        {
            return new universemaponly.meshData(currentPosition, 0, positions, triangleIndices, smallX, smallY, smallZ);
        }
    }
    ...
    public void DrawBrick(int x, int y, int z, Vector3 currentPosition)
    {
        ...
        /*if (y == 0)
        {
            ...
            if (smallChunkTop != null)
            {
                if (smallChunk.getSmallChunkTop(posChunkTopX, posChunkTopY, posChunkTopZ).Getint(x, 0, z) == 0)
                {
                    //TOPFACE
                    if (IsTransparent(x, y + 1, z))
                    {
        ...
        /* if (y == height-1 && infini.getBigChunk(new Vector3(currentPosition.X, currentPosition.Y-10, currentPosition.Z), chunker) != null)
         {
             ...
             if (chuk.chunkerList[xPoser,yPoser,zPoser].Getint(x, y, width - 1) == 0)
             {
                 //TOPFACE
                 if (IsTransparent(x, y + 1, z))
                 {
        ...
        if (y == 0 && test.getChunk(transform.position.x, transform.position.y, transform.position.z - 1) != null)
        {
            ...
            if (chuk.chunker.GetComponent<chunku>().Getint(x, y, width - 1) == 0)
            {
                //TOPFACE
                if (IsTransparent(x, y + 1, z))
                {
