...
public class universemapvertSLOWrev : MonoBehaviour
{
    ...
    public class testerOfNumber
    {
        ...
        int detailScale = 5;
        int heightScale = 5;
        public static Queue<mainChunk> firstQueue = new Queue<mainChunk>();
        ...
        public class checkingBytes
        {
            ...
            public checkingBytes(Vector3 smallChunkPos, int smallX, int smallY, int smallZ, bool resultsOne, bool resultsTwo, bool resultsThree, smallChunker[] worldChunk, int[] map, int indexx,int indexy,int indexz) //, GameObject thegameobject) // sccscomputevoxelshrinkedALLFACESslowREV sccscomputevoxelshrinkedALLFACESslowREVshrinked_,
            {
                ...
                this.indexy = indexy;
                this.indexz = indexz;
                //this.thegameobject = thegameobject;
            }
        }*/
        ...
        public class mainChunk
        {
            ...
            public mapofints[][] datamapfirstvertxtop;
            public mapofints[][] datamapfirstvertytop;
            public mapofints[][] datamapfirstvertztop;
            public mapofints[][] datawidthdimtop;
            ...
            public ComputeBuffer mapvertlocbufferx;
            public ComputeBuffer mapvertlocbuffery;
            public ComputeBuffer mapvertlocbufferz;

            public ComputeBuffer mapwidthdimtop;
            public ComputeBuffer mapheightdimtop;
            public ComputeBuffer mapdepthdimtop;
                 ...
                 mapofints[][] datawidthdimtop, mapofints[][] dataheightdimtop, mapofints[][] datadepthdimtop) //, GameObject thegameobject) // sccscomputevoxelshrinkedALLFACESslowREV sccscomputevoxelshrinkedALLFACESslowREVshrinked_,
            {
                ...
                this.datamapfirstvertxtop = datamapfirstvertxtop;
                this.datamapfirstvertytop = datamapfirstvertytop;
                this.datamapfirstvertztop = datamapfirstvertztop;
                this.datawidthdimtop = datawidthdimtop;
                this.dataheightdimtop = dataheightdimtop;
                this.datadepthdimtop = datadepthdimtop;
        ...
        void smallChunkCreator(Vector3 viewerPosition, smallChunker[] worldChunk)
        {
            ...
            int sizerX = smallChunkWidth;
            int sizerY = smallChunkWidth;
            int sizerZ = smallChunkWidth;

            for (int xx = -sizerX; xx < sizerX + sizerX; xx += smallChunkWidth)
            {
                for (int yy = -sizerY; yy < sizerY + sizerY; yy += smallChunkWidth)
                {
                    for (int zz = -sizerZ; zz < sizerZ + sizerZ; zz += smallChunkWidth)
                    {
                        ...
                        {
                            indexx *= -1;
                            indexx = (sizerZ + sizerZ) + indexx;
                        }
                        if (indexy < 0)
                        {
                            indexy *= -1;
                            indexy = (sizerY + sizerY) + indexy;
                        }
                        if (indexz < 0)
                        {
                            indexz *= -1;
                            indexz = (sizerX + sizerX) + indexz;
                        }
                        ...
                        getPosSmall(xx, yy, zz, out smallX, out smallY, out smallZ, smallChunkWidth, smallChunkWidth, smallChunkWidth, 2);

                        int smallerX;
                        int smallerY;
                        int smallerZ;

                        getPosSmall(xx, yy, zz, out smallerX, out smallerY, out smallerZ, smallChunkWidth, smallChunkWidth, smallChunkWidth, 2);
                        ...
                        lock (tempSmallWrite)
                        {
                            if (worldChunk[smallX + width * (smallerY + height * smallZ)] == null)
                            {
                                ...
                                //mainChunk checkBytes = new mainChunk(realPos, smallX, smallerY, smallZ, resultsOne, resultsTwo, resultsThree, worldChunk, null,xx,yy,zz, null, null, null, null, null, null,null); //null

                                mainChunk checkBytes = new mainChunk();
                                checkBytes.smallChunkPos = realPos;
                                checkBytes.smallX = smallX;
                                checkBytes.smallY = smallY;
                                checkBytes.smallZ = smallZ;

                                checkBytes.resultsOne = resultsOne;
                                checkBytes.resultsTwo = resultsTwo;
                                checkBytes.resultsThree = resultsThree;

                                checkBytes.worldChunk = worldChunk;

                                checkBytes.map = null;
                                checkBytes.indexx = xx;
                                checkBytes.indexy = yy;
                                ...
                                checkBytes.datawidthdimtop = null;
                                checkBytes.dataheightdimtop = null;
                                checkBytes.datadepthdimtop = null;
        ...
        void checkLastBytes(object stateInfo)
        {
            ...
            Queue<mainChunk> queuer = (Queue<mainChunk>)stateInfo;
            //lock (queuer)
            {
                for (int i = 0; i < queuer.Count; i++)
                {
                    ...
                    mainChunk mainChunk = queuer.Dequeue();

                    Vector3 smallChunkPos = mainChunk.smallChunkPos;
    ...
    IEnumerator buildChunks()
    {
        ...
        {
            testerOfNumber.mainChunk byteCheck = testerOfNumber.nextQueue.Dequeue();
            if (byteCheck != null)
            {
                ...
                if (!resultsOne && !resultsThree && !resultsTwo)
                {
                    //if (byteCheck.worldChunk[byteCheck.smallX + realChunkWidth * (byteCheck.smallY + realChunkHeight * byteCheck.smallZ)] == null)
                    {
                        ...
                        */
                        ...
                        //byteCheck.ch
                        //byteCheck.thegameobject = currentChunk;
                        ...
                        */
                        ...
                        universemaponly.meshData meshDator = chunky.startBuildingArray(byteCheck.smallChunkPos, byteCheck.smallX, byteCheck.smallY, byteCheck.smallZ, byteCheck.map);
                        ...
                        */
                        ...
                        /*
                        chunk chunky = new chunk();
                        universemaponly.meshData meshDator = chunky.startBuildingArray(byteCheck.smallChunkPos, byteCheck.smallX, byteCheck.smallY, byteCheck.smallZ, themap);
                        currentChunk.transform.position = byteCheck.smallChunkPos;// transform.position;
                        currentChunk.transform.rotation = transform.rotation;
                        currentChunk.SetActive(true);
                        currentChunk.transform.parent = (Transform)GameObject.FindGameObjectWithTag("terrain").transform;
