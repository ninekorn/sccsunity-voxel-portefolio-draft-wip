...
public class collisionatorManager : MonoBehaviour {
    ...
    public void collisionForceToApply(Transform objectToApplyForce,Vector3 collisionPoint,Vector3 velocity,Vector3 directionOfCollision,Vector3 verticePointOfOtherObject,int startTriangleIndex,int verticeColIndex,Transform objectThatCollidesWithMe)
    {
        //SetInterval(() => Debug.Log("Hello Stackoverflow"), TimeSpan.FromSeconds(4));
        ...
        if (forceApplied > 0.25f)
        {
            for (int n = 0; n < triangles.Length; n += 3)
            {
                ...
                for (int i = 0; i < trianglesOfObjectThatCollidesWithMe.Length; i += 3)
                {
                    ...
                    if (TriTriOverlap.TriTriIntersect(p00, p01, p02, pointOne, pointTwo, pointThree))
                    {
                        ...
                        pointOne += velocity.normalized * (forceApplied * 0.1f) * 0.0001f;
                        pointTwo += velocity.normalized * (forceApplied * 0.1f) * 0.0001f;
                        pointThree += velocity.normalized * (forceApplied * 0.1f) * 0.0001f;
            ...
            p0 += velocity.normalized * (forceApplied * 0.1f) * 0.0001f;
            p1 += velocity.normalized * (forceApplied * 0.1f) * 0.0001f;
            p2 += velocity.normalized * (forceApplied * 0.1f) * 0.0001f;
